...
export class RutasFormPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.authService.isAuth2().subscribe(auth => {
      if (auth) {
        this.authService.isUser(auth.uid).subscribe(user => {
          this.authService.getUsersByServerName(user.server.name)
          .subscribe(users => {
            const employees = users.filter(user => {
              const userData = user.payload.doc.data() as User;
              return userData.server.employee;
            });
            this.techniciansList = employees.map(user => {
              return {
